...
require([], function () {
	var waterfall = {
		init: function() {
			waterfall._waterfall('bigbox', 'smallbox');
		},
		_waterfall: function(parent, box) {
			// 将bigbox下的smallbox全部取出
			var oParent = document.getElementById(parent);
			// 根据class获取该class下元素
			var oBoxs = waterfall._getByClass(oParent, box);
			//获取small盒子宽度
			var oBoxW = oBoxs[0].offsetWidth;
			// 计算列数(bigbox.width/small.width)并取整
			var cols = parseInt(oParent.offsetWidth / oBoxW);
			var hArr = [];
			for (var i = 0; i < oBoxs.length; i++) {
				if (i < cols) {
					...
					var index = waterfall._getMinHeightIndex(hArr, minH); // 获取height最小时对应的列数
		...
		_getByClass: function(parent, clsName) {
			// 根据class获取该class下元素
			var boxArr = [];
			// 获取parent下的所有元素
		...
		_getMinHeightIndex: function(arr, val) {
			// 获取height最小时对应的列数
